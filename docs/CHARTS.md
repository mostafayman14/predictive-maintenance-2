...
4. [Data Flow (History → Chart Store → Live Append)](#4-data-flow-history--chart-store--live-append)
...
8. [The 3-Hour Rolling Window](#8-the-3-hour-rolling-window)
...
Each chart is a **time-series line chart** that keeps a **3-hour rolling window** of readings and appends live points as they arrive.

```mermaid
flowchart TB
  subgraph sources [Data Sources]
    REST["GET /api/v1/history (once)"]
    Poll["POST /api/v1/live (polled ~1s)"]
  end

  subgraph store [Chart Memory]
    ChartCtx["ChartDataContext"]
  end
...
**Key idea:** chart series live in `ChartDataContext`. History loads once; each live poll appends points immutably and trims to 3 hours. UI labels come from `dashboardUiConfig.js` (no mock chart seeds).
...
|-----------|--------|-----------|
| `points`  | `[{ timestamp, value }]` | Primary — history API + live append |
| `dataset` | Same as `points` | Alias accepted by `LiveLineChart` |

---

## 4. Data Flow (History → Chart Store → Live Append)

### Step 1 — Initial state (empty shells)

**File:** `src/data/dashboardUiConfig.js` / `createEmptyCharts()` in `chartUtils.js`

On first load the charts hold only metadata and **no points** — waiting UI until history/live data arrives.

### Step 2 — History load (once)

**File:** `src/context/ChartDataContext.jsx`

On mount, `GET /history` sanitizes and trims each series into the chart store. Empty `points: []` from the backend is expected (frontend memory owns the series after that).

### Step 3 — Live append

**File:** `src/context/LiveDataContext.jsx` → `appendLiveReading`

Each successful live poll updates prediction/sensor cards **and** appends one point per sensor into the chart store (immutable), then trims to 3 hours. Chart series are **not** re-merged from history on every render.

### Step 4 — React re-render

`useDashboardData()` reads `chartData.charts` from context. When the store updates:

1. `DashboardPage` passes `data.charts` to `LiveChartsSection`
2. Each `LiveLineChart` normalizes points and Recharts redraws

```mermaid
sequenceDiagram
  participant Poll as livePollingService
  participant Pi as Raspberry Pi / Backend
  participant Live as LiveDataContext
  participant Charts as ChartDataContext
  ...
  Poll->>Live: onMessage(payload)
  Live->>Live: createLivePatch()
  Live->>Charts: appendLiveReading(payload)
  Charts->>Charts: appendChartPoint + trim 3h
  Hook->>Chart: charts from ChartDataContext
...
```js
CHART_WINDOW_MS = 3 * 60 * 60 * 1000  // 3 hours
CHART_WINDOW_SECONDS = CHART_WINDOW_MS / 1000
CHART_WINDOW_HOURS = 3
```

Every chart shows at most **3 hours** of data.

### `sanitizePoints(points)`

Keeps only finite `timestamp` + finite `value`, sorts ascending, and collapses duplicate timestamps (last wins).
...
```js
windowStart = endTime - CHART_WINDOW_MS
keep points where: windowStart <= timestamp <= endTime
```

Then runs `sanitizePoints`. Old points fall off the left side automatically.

### `normalizeChartDataset(dataset)`

Called inside `LiveLineChart` on every render (memoized). Sanitizes and trims against `Date.now()`. Empty input → `[]` (waiting UI).
...
Skips invalid values and ignores `timestamp <= last.timestamp`.

### `mergeChartPoints(existingPoints, incomingChart, scalarValue, timestamp)`

Used when replacing or appending series. Priority:

1. **`incomingChart.points`** — replace with sanitized+trimmed points
2. **`scalarValue`** — append single reading (typical live-poll case)
3. Otherwise — keep sanitized existing points
...
### Chart append (owned by `ChartDataContext.appendLiveReading`)

For each sensor key, appends a point (or replaces when `charts[key].points` is provided), keeps unit `A` for current, and trims to 3 hours.
...
Points keep accumulating until the 3-hour window fills, after which the oldest points are trimmed away.
...
- Type: `number` (Unix ms)
- Domain: fixed 3-hour window from `getChartWindowDomain()`
- `allowDataOverflow` — points outside domain are clipped, window slides forward

---

## 8. The 3-Hour Rolling Window
...
1. Append to array (if timestamp is newer than last)
2. Remove points older than `windowEnd - 3h`
3. X-axis domain shifts right → **auto-scroll effect**
...
├── data/dashboardUiConfig.js        # UI labels + empty chart shells
├── context/
│   ├── ChartDataContext.jsx         # history load + live append + 3h trim
│   └── LiveDataContext.jsx          # polling + livePatch + chart append hook
├── services/
│   ├── dashboardNormalizer.js       # status/live → cards (charts from store)
│   ├── liveDataNormalizer.js        # live payload → sensor/condition patch
...
createLivePatch() stores per-sensor { value, timestamp } objects
        ↓
appendLiveReading() → appendChartPoint(existing, 68, sensorTimestamp)
        ↓
appendChartPoint() dedups if timestamp <= last point, else appends
        ↓
trimChartWindow() keeps last 3h
        ↓
useDashboardData() → data.charts.temperature.points from ChartDataContext
        ↓
...
| Time window | 3 hours rolling |
| Point format | `{ timestamp: ms, value: number }` |
| Live updates | Per-sensor `{ timestamp, value }` appended per poll of `POST /live` |
| Initial data | Empty charts + `GET /history` into `ChartDataContext` |
| Display value | Last point, or sensor fallback |
| Main merge logic | `chartUtils.js` + `ChartDataContext` |
| UI entry point | `LiveChartsSection` → `SensorLineChart` → `LiveLineChart` |

If you extend the project (add a 5th sensor, change window length, etc.), start with `chartConfig.js` / `chartUtils.js` and mirror the same keys in `ChartDataContext` and `dashboardUiConfig.js`.
